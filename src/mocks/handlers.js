...
export const handler = [
  rest.get('/getLikeList', (req, res, ctx) =>{
    const { pageNum } = req.body
    function createData(pageNum) {
      return [
        {
          id: (pageNum - 1) * 4,
    ...
    return res(
      ctx.status(200),
      ctx.json(createData(pageNum))
    )
  }),

  rest.get('/getShopCartList', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json([
        {
  ...
  rest.get('/getWantList', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json([
        {
